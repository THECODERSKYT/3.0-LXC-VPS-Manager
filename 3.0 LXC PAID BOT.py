...
DISCORD_TOKEN = os.getenv('MTQ1NzA5NDc1NjM3ODM0NTUxNA.GOS7oa.KLk8ESym4kP2Cu2EvPczFsjoF6UO7kDjhRHyvM')
MAIN_ADMIN_ID = int(os.getenv('1193518156065538068'))
VPS_USER_ROLE_ID = int(os.getenv('1456154828399972486'))
...
logger = logging.getLogger('GloryVPS_vps_bot')
...
# Embed creation functions with black theme and GloryVPS branding
def create_embed(title, description="", color=0x1a1a1a):
    embed = discord.Embed(
        title=truncate_text(f"⭐ GloryVPS - {title}", 256),
        description=truncate_text(description, 4096),
        color=color
    )
    embed.set_thumbnail(url="https://i.ibb.co/7NVfcBv9/Chat-GPT-Image-Jan-4-2026-01-10-52-AM.png")
    embed.set_footer(text=f"Glory VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                     icon_url="https://i.ibb.co/7NVfcBv9/Chat-GPT-Image-Jan-4-2026-01-10-52-AM.png")
    return embed
...
def is_admin():
    async def predicate(ctx):
        ...
        if user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", []):
            return True
        raise commands.CheckFailure("You need admin permissions to use this command. Contact GloryVPS support.")
...
async def get_or_create_vps_role(guild):
    ...
    if VPS_USER_ROLE_ID:
        ...
        if role:
            return role
    role = discord.utils.get(guild.roles, name="GloryVPS User")
    if role:
    ...
    try:
        role = await guild.create_role(
            ...
            reason="GloryVPS User role for bot management",
            permissions=discord.Permissions.none()
        )
        VPS_USER_ROLE_ID = role.id
        logger.info(f"Created GloryVPS VPS User role: {role.name} (ID: {role.id})")
        return role
    except Exception as e:
        logger.error(f"Failed to create GloryVPS User role: {e}")
        return None
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Glory VPS Manager"))
    logger.info("GloryVPS Bot is ready!")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        error_msg = str(error) if str(error) else "You need admin permissions for this command. Contact GloryVPS support."
        ...
        await ctx.send(embed=create_error_embed("System Error", "An unexpected error occurred. GloryVPS support has been notified."))

# Bot commands
@bot.command(name='ping')
async def ping(ctx):
    latency = round(bot.latency * 1000)
    embed = create_success_embed("Pong!", f"GloryVPS Bot latency: {latency}ms")
...
async def my_vps(ctx):
    ...
    if not vps_list:
        embed = create_error_embed("No VPS Found", "You don't have any Glory VPS. Contact an admin to create one.")
        add_field(embed, "Quick Actions", "• `!manage` - Manage VPS\n• Contact GloryVPS admin for VPS creation", False)
        await ctx.send(embed=embed)
        return
    embed = create_info_embed("My GloryVPS VPS", "")
    text = []
...
async def lxc_list(ctx):
    try:
        ...
        embed = create_info_embed("GloryVPS LXC Containers List", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        container_name = f"GloryVPS-{user_id}-{vps_count}"
        ...
        try:
            ...
            if self.ctx.guild:
                ...
                if vps_role:
                    try:
                        await self.user.add_roles(vps_role, reason="GloryVPS ownership granted")
                    except discord.Forbidden:
                        logger.warning(f"Failed to assign GloryVPS role to {self.user.name}")
            success_embed = create_success_embed("GloryVPS Created Successfully")
            ...
            dm_embed = create_success_embed("GloryVPS Created!", f"Your VPS has been successfully deployed by an admin!")
            add_field(dm_embed, "VPS Details", f"**VPS ID:** #{vps_count}\n**Container Name:** `{container_name}`\n**Configuration:** {config_str}\n**Status:** Running\n**OS:** {os_version}\n**Created:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", False)
            add_field(dm_embed, "Management", "• Use `!manage` to start/stop/reinstall your GloryVPS\n• Use `!manage` → SSH for terminal access\n• Contact GloryVPS admin for upgrades or issues", False)
...
class ManageView(discord.ui.View):
        ...
        if len(vps_list) > 1:
            options = [
                discord.SelectOption(
                    label=f"GloryVPS {i+1} ({v.get('config', 'Custom')})",
                    description=f"Status: {v.get('status', 'unknown')}",
                    value=str(i)
                ) for i, v in enumerate(vps_list)
            ]
            self.select = discord.ui.Select(placeholder="Select a GloryVPS to manage", options=options)
            self.select.callback = self.select_vps
            self.add_item(self.select)
            self.initial_embed = create_embed("GloryVPS Management", "Select a VPS from the dropdown menu below.", 0x1a1a1a)
    ...
    async def create_vps_embed(self, index):
        ...
        embed = create_embed(
            f"GloryVPS Management - VPS {index + 1}",
        ...
        if suspended:
            add_field(embed, "⚠️ Suspended", "This GloryVPS is suspended. Contact an admin to unsuspend.", False)
        if whitelisted:
        ...
        add_field(embed, "🎮 Controls", "Use the buttons below to manage your GloryVPS", False)
        return embed
    ...
    async def select_vps(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This is not your GloryVPS!"), ephemeral=True)
            return
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if suspended and not self.is_admin and action != 'stats':
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This GloryVPS is suspended. Contact an admin to unsuspend."), ephemeral=True)
            return
        ...
        if action == 'stats':
            ...
            disk_usage = await get_container_disk(container_name)
            uptime = await get_container_uptime(container_name)
            stats_embed = create_info_embed("📈 GloryVPS Live Statistics", f"Real-time stats for `{container_name}`")
        ...
        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the GloryVPS VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Cannot Reinstall", "Unsuspend the GloryVPS first."), ephemeral=True)
                return
            os_version = target_vps.get('os_version', 'ubuntu:22.04')
            confirm_embed = create_warning_embed("GloryVPS Reinstall Warning",
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, inter: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        await inter.followup.send(embed=create_info_embed("Recreating Container", f"Creating new GloryVPS container `{self.container_name}`..."), ephemeral=True)
                        ...
                        save_vps_data()
                        await inter.followup.send(embed=create_success_embed("Reinstall Complete", f"GloryVPS `{self.container_name}` has been successfully reinstalled!"), ephemeral=True)
        ...
        if action == 'start':
            try:
                ...
                target_vps["status"] = "running"
                save_vps_data()
                await interaction.followup.send(embed=create_success_embed("VPS Started", f"GloryVPS `{container_name}` is now running!"), ephemeral=True)
            except Exception as e:
            ...
            try:
                ...
                save_vps_data()
                await interaction.followup.send(embed=create_success_embed("VPS Stopped", f"GloryVPS `{container_name}` has been stopped!"), ephemeral=True)
            except Exception as e:
                await interaction.followup.send(embed=create_error_embed("Stop Failed", str(e)), ephemeral=True)
        elif action == 'tmate':
            if suspended:
                await interaction.followup.send(embed=create_error_embed("Access Denied", "Cannot access suspended GloryVPS."), ephemeral=True)
                return
            await interaction.followup.send(embed=create_info_embed("SSH Access", "Generating GloryVPS SSH connection..."), ephemeral=True)
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await interaction.followup.send(embed=create_success_embed("Installed", "GloryVPS SSH service installed!"), ephemeral=True)
                session_name = f"GloryVPS-session-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if ssh_url:
                    try:
                        ssh_embed = create_embed("🔑 GloryVPS SSH Access", f"SSH connection for VPS `{container_name}`:", 0x00ff88)
                        ...
                        await interaction.followup.send(embed=create_success_embed("SSH Sent", f"Check your DMs for GloryVPS SSH link! Session: {session_name}"), ephemeral=True)
                    except discord.Forbidden:
                        await interaction.followup.send(embed=create_error_embed("DM Failed", "Enable DMs to receive GloryVPS SSH link!"), ephemeral=True)
                else:
...
async def manage_vps(ctx, user: discord.Member = None):
    if user:
        ...
        if user_id_check != str(MAIN_ADMIN_ID) and user_id_check not in admin_data.get("admins", []):
            await ctx.send(embed=create_error_embed("Access Denied", "Only GloryVPS admins can manage other users' VPS."))
            return
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("No VPS Found", f"{user.mention} doesn't have any GloryVPS."))
            return
        view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
        await ctx.send(embed=create_info_embed(f"Managing {user.name}'s GloryVPS", f"Managing VPS for {user.mention}"), view=view)
    else:
        user_id = str(ctx.author.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_error_embed("No VPS Found", "You don't have any GloryVPS. Contact an admin to create one.")
            add_field(embed, "Quick Actions", "• `!manage` - Manage VPS\n• Contact GloryVPS admin for VPS creation", False)
            await ctx.send(embed=embed)
            return
...
async def list_all_vps(ctx):
    ...
    for user_id, vps_list in vps_data.items():
        try:
            ...
            user_summary.append(f"**{user.name}** ({user.mention}) - {user_vps_count} GloryVPS ({user_running} running, {user_suspended} suspended, {user_whitelisted} whitelisted)")
            ...
            vps_info.append(f"❓ Unknown User ({user_id}) - {len(vps_list)} GloryVPS")
    embed = create_embed("All GloryVPS Information", "Complete overview of all GloryVPS deployments and user statistics", 0x1a1a1a)
    add_field(embed, "System Overview", f"**Total Users:** {total_users}\n**Total VPS:** {total_vps}\n**Running:** {running_vps}\n**Stopped:** {stopped_vps}\n**Suspended:** {suspended_vps}\n**Whitelisted:** {whitelisted_vps}", False)
    await ctx.send(embed=embed)
    if user_summary:
        embed = create_embed("GloryVPS User Summary", f"Summary of all users and their GloryVPS", 0x1a1a1a)
        summary_text = "\n".join(user_summary)
    ...
    if vps_info:
        ...
        for idx, chunk in enumerate(chunks, 1):
            embed = create_embed(f"GloryVPS Details (Part {idx})", "List of all GloryVPS deployments", 0x1a1a1a)
...
async def manage_shared_vps(ctx, owner: discord.Member, vps_number: int):
    ...
    if owner_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[owner_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or owner doesn't have a GloryVPS."))
        return
    vps = vps_data[owner_id][vps_number - 1]
    if user_id not in vps.get("shared_with", []):
        await ctx.send(embed=create_error_embed("Access Denied", "You do not have access to this GloryVPS."))
        return
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    ...
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a GloryVPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    if "shared_with" not in vps:
        vps["shared_with"] = []
    if shared_user_id in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Already Shared", f"{shared_user.mention} already has access to this GloryVPS!"))
        return
    vps["shared_with"].append(shared_user_id)
    save_vps_data()
    await ctx.send(embed=create_success_embed("VPS Shared", f"GloryVPS #{vps_number} shared with {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("GloryVPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `!manage-shared {ctx.author.mention} {vps_number}`", 0x00ff88))
    except discord.Forbidden:
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    ...
    if shared_user_id not in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Not Shared", f"{shared_user.mention} doesn't have access to this GloryVPS!"))
        return
    vps["shared_with"].remove(shared_user_id)
    save_vps_data()
    await ctx.send(embed=create_success_embed("Access Revoked", f"Access to GloryVPS #{vps_number} revoked from {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("GloryVPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    user_id = str(user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or user doesn't have a GloryVPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    container_name = vps["container_name"]
    await ctx.send(embed=create_info_embed("Deleting GloryVPS", f"Removing VPS #{vps_number}..."))
    try:
        ...
        if not vps_data[user_id]:
            ...
            if ctx.guild:
                ...
                if vps_role and vps_role in user.roles:
                    try:
                        await user.remove_roles(vps_role, reason="No GloryVPS ownership")
                    except discord.Forbidden:
                        logger.warning(f"Failed to remove GloryVPS role from {user.name}")
        save_vps_data()
        embed = create_success_embed("GloryVPS Deleted Successfully")
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    ...
    for uid, vps_list in vps_data.items():
        ...
        if found_vps:
            break
    if not found_vps:
        await ctx.send(embed=create_error_embed("VPS Not Found", f"No GloryVPS found with ID: `{vps_id}`"))
        return
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    disk_changed = disk is not None
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping GloryVPS `{vps_id}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("Resources Added", f"Successfully added resources to GloryVPS `{vps_id}`")
...
async def admin_add(ctx, user: discord.Member):
    ...
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Already Admin", "This user is already the main GloryVPS admin!"))
        return
    if user_id in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Already Admin", f"{user.mention} is already a GloryVPS admin!"))
        return
    admin_data["admins"].append(user_id)
    save_admin_data()
    await ctx.send(embed=create_success_embed("Admin Added", f"{user.mention} is now a GloryVPS admin!"))
    try:
        await user.send(embed=create_embed("🎉 GloryVPS Admin Role Granted", f"You are now a GloryVPS admin by {ctx.author.mention}", 0x00ff88))
    except discord.Forbidden:
...
async def admin_remove(ctx, user: discord.Member):
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Cannot Remove", "You cannot remove the main GloryVPS admin!"))
        return
    if user_id not in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Not Admin", f"{user.mention} is not a GloryVPS admin!"))
        return
    admin_data["admins"].remove(user_id)
    save_admin_data()
    await ctx.send(embed=create_success_embed("Admin Removed", f"{user.mention} is no longer a GloryVPS admin!"))
    try:
        await user.send(embed=create_embed("⚠️ GloryVPS Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_list(ctx):
    ...
    embed = create_embed("👑 GloryVPS Admin Team", "Current GloryVPS administrators:", 0x1a1a1a)
    ...
    if admins:
        ...
        add_field(embed, "🛡️ Admins", "No additional GloryVPS admins", False)
    await ctx.send(embed=embed)
...
async def user_info(ctx, user: discord.Member):
    user_id = str(user.id)
    vps_list = vps_data.get(user_id, [])
    embed = create_embed(f"GloryVPS User Information - {user.name}", f"Detailed information for {user.mention}", 0x1a1a1a)
    ...
    if vps_list:
        ...
        add_field(embed, "🖥️ GloryVPS Information", vps_summary, False)
        ...
        add_field(embed, "🖥️ GloryVPS Information", "**No VPS owned**", False)
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])
    add_field(embed, "🛡️ GloryVPS Admin Status", f"**{'Yes' if is_admin_user else 'No'}**", False)
    await ctx.send(embed=embed)
...
async def server_stats(ctx):
    ...
    for vps_list in vps_data.values():
        for vps in vps_list:
            ...
            if vps.get('whitelisted', False):
                whitelisted_vps += 1
    embed = create_embed("📊 GloryVPS Server Statistics", "Current GloryVPS server overview", 0x1a1a1a)
    ...
    if not container_name:
        ...
        for user_id, vps_list in vps_data.items():
            try:
                ...
                for i, vps in enumerate(vps_list):
                    ...
                    if vps.get('whitelisted', False):
                        status_text += " (WHITELISTED)"
                    all_vps.append(f"**{user.name}** - GloryVPS {i+1}: `{vps['container_name']}` - {status_text}")
            except:
                pass
        vps_text = "\n".join(all_vps)
        chunks = [vps_text[i:i+1024] for i in range(0, len(vps_text), 1024)]
        for idx, chunk in enumerate(chunks, 1):
            embed = create_embed(f"🖥️ All GloryVPS (Part {idx})", f"List of all GloryVPS deployments", 0x1a1a1a)
            add_field(embed, "VPS List", chunk, False)
            await ctx.send(embed=embed)
    else:
        found_vps = None
        ...
        for user_id, vps_list in vps_data.items():
            ...
            if found_vps:
                break
        if not found_vps:
            await ctx.send(embed=create_error_embed("VPS Not Found", f"No GloryVPS found with container name: `{container_name}`"))
            return
        ...
        embed = create_embed(f"🖥️ GloryVPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}{whitelisted_text}", 0x1a1a1a)
...
async def restart_vps(ctx, container_name: str):
    await ctx.send(embed=create_info_embed("Restarting VPS", f"Restarting GloryVPS `{container_name}`..."))
    try:
        ...
        for user_id, vps_list in vps_data.items():
            for vps in vps_list:
                if vps['container_name'] == container_name:
                    ...
                    save_vps_data()
                    break
        await ctx.send(embed=create_success_embed("VPS Restarted", f"GloryVPS `{container_name}` has been restarted successfully!"))
    except Exception as e:
...
async def execute_command(ctx, container_name: str, *, command: str):
    await ctx.send(embed=create_info_embed("Executing Command", f"Running command in GloryVPS `{container_name}`..."))
    try:
        proc = await asyncio.create_subprocess_exec(
...
async def stop_all_vps(ctx):
    embed = create_warning_embed("Stopping All GloryVPS", "⚠️ **WARNING:** This will stop ALL running VPS on the GloryVPS server.\n\nThis action cannot be undone. Continue?")
    class ConfirmView(discord.ui.View):
        def __init__(self):
            super().__init__(timeout=60)
                    ...
                    for user_id, vps_list in vps_data.items():
                        for vps in vps_list:
                            if vps.get('status') == 'running':
                                ...
                                stopped_count += 1
                    save_vps_data()
                    embed = create_success_embed("All GloryVPS Stopped", f"Successfully stopped {stopped_count} VPS using `lxc stop --all --force`")
                    output_text = stdout.decode() if stdout else 'No output'
                    add_field(embed, "Command Output", f"```\n{output_text}\n```", False)
                    await interaction.followup.send(embed=embed)
                else:
                    error_msg = stderr.decode() if stderr else "Unknown error"
                    embed = create_error_embed("Stop Failed", f"Failed to stop GloryVPS: {error_msg}")
                    await interaction.followup.send(embed=embed)
            except Exception as e:
        ...
        async def cancel(self, interaction: discord.Interaction, item: discord.ui.Button):
            await interaction.response.edit_message(embed=create_info_embed("Operation Cancelled", "The stop all GloryVPS operation has been cancelled."))
...
async def resource_monitor_control(ctx, action: str = "status"):
    ...
    if action.lower() == "status":
        ...
        embed = create_embed("GloryVPS Resource Monitor Status", f"GloryVPS resource monitoring is currently **{status}**", 0x00ccff if resource_monitor_active else 0xffaa00)
        ...
        resource_monitor_active = True
        await ctx.send(embed=create_success_embed("Resource Monitor Enabled", "GloryVPS resource monitoring has been enabled."))
    elif action.lower() == "disable":
        resource_monitor_active = False
        await ctx.send(embed=create_warning_embed("Resource Monitor Disabled", "GloryVPS resource monitoring has been disabled."))
    else:
...
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    ...
    disk_changed = disk is not None
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping GloryVPS `{container_name}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("VPS Resized", f"Successfully resized resources for GloryVPS `{container_name}`")
        add_field(embed, "Changes Applied", "\n".join(changes), False)
        if disk_changed:
            add_field(embed, "Disk Note", "Run `sudo resize2fs /` inside the VPS to expand the filesystem.", False)
        await ctx.send(embed=embed)
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    if not new_name:
        ...
        new_name = f"GloryVPS-{container_name}-clone-{timestamp}"
    await ctx.send(embed=create_info_embed("Cloning VPS", f"Cloning GloryVPS `{container_name}` to `{new_name}`..."))
    try:
        found_vps = None
        user_id = None
        ...
        embed = create_success_embed("VPS Cloned", f"Successfully cloned GloryVPS `{container_name}` to `{new_name}`")
...
async def migrate_vps(ctx, container_name: str, target_pool: str):
    await ctx.send(embed=create_info_embed("Migrating VPS", f"Migrating GloryVPS `{container_name}` to storage pool `{target_pool}`..."))
    try:
        await execute_lxc(f"lxc stop {container_name}")

        temp_name = f"GloryVPS-{container_name}-temp-{int(time.time())}"
        ...
        await ctx.send(embed=create_success_embed("VPS Migrated", f"Successfully migrated GloryVPS `{container_name}` to storage pool `{target_pool}`"))
...
async def vps_stats(ctx, container_name: str):
    await ctx.send(embed=create_info_embed("Gathering Statistics", f"Collecting statistics for GloryVPS `{container_name}`..."))
    try:
        status = await get_container_status(container_name)
        cpu_usage = await get_container_cpu(container_name)
        memory_usage = await get_container_memory(container_name)
        ...
        embed = create_embed(f"📊 GloryVPS Statistics - {container_name}", f"Resource usage statistics", 0x1a1a1a)
    ...
    try:
        if action.lower() == "list":
            ...
            if proc.returncode == 0:
                ...
                embed = create_embed(f"🌐 GloryVPS Network Interfaces - {container_name}", "Network configuration", 0x1a1a1a)
            ...
            await ctx.send(embed=create_success_embed("Network Limited", f"Set GloryVPS network limit to {value} for `{container_name}`"))
            ...
            await ctx.send(embed=create_success_embed("Network Added", f"Added network interface to GloryVPS `{container_name}` with bridge `{value}`"))

        elif action.lower() == "remove" and value:
            await execute_lxc(f"lxc config device remove {container_name} {value}")
            await ctx.send(embed=create_success_embed("Network Removed", f"Removed network interface `{value}` from GloryVPS `{container_name}`"))
...
async def vps_processes(ctx, container_name: str):
    await ctx.send(embed=create_info_embed("Gathering Processes", f"Listing processes in GloryVPS `{container_name}`..."))
    try:
        proc = await asyncio.create_subprocess_exec(
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"⚙️ GloryVPS Processes - {container_name}", "Running processes", 0x1a1a1a)
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    await ctx.send(embed=create_info_embed("Gathering Logs", f"Fetching last {lines} lines from GloryVPS `{container_name}`..."))
    try:
        proc = await asyncio.create_subprocess_exec(
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"📋 GloryVPS Logs - {container_name}", f"Last {lines} log lines", 0x1a1a1a)
...
async def suspend_vps(ctx, container_name: str, *, reason: str = "Admin action"):
    ...
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if vps.get('status') != 'running':
                    await ctx.send(embed=create_error_embed("Cannot Suspend", "GloryVPS must be running to suspend."))
                    return
                try:
                    await execute_lxc(f"lxc stop {container_name}")
                    vps['status'] = 'stopped'
                ...
                try:
                    ...
                    embed = create_warning_embed("🚨 GloryVPS Suspended", f"Your VPS `{container_name}` has been suspended by an admin.\n\n**Reason:** {reason}\n\nContact a GloryVPS admin to unsuspend.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
                    logger.error(f"Failed to DM owner {uid}: {dm_e}")
                await ctx.send(embed=create_success_embed("VPS Suspended", f"GloryVPS `{container_name}` suspended. Reason: {reason}"))
                found = True
                break
        if found:
            break
    if not found:
        await ctx.send(embed=create_error_embed("Not Found", f"GloryVPS `{container_name}` not found."))
        ...
        for vps in lst:
            if vps['container_name'] == container_name:
                if not vps.get('suspended', False):
                    await ctx.send(embed=create_error_embed("Not Suspended", "GloryVPS is not suspended."))
                    return
                try:
                    vps['suspended'] = False
                    vps['status'] = 'running'
                    await execute_lxc(f"lxc start {container_name}")
                    save_vps_data()
                    await ctx.send(embed=create_success_embed("VPS Unsuspended", f"GloryVPS `{container_name}` unsuspended and started."))
                    found = True
                except Exception as e:
                    await ctx.send(embed=create_error_embed("Start Failed", str(e)))
                try:
                    owner = await bot.fetch_user(int(uid))
                    embed = create_success_embed("🟢 GloryVPS Unsuspended", f"Your VPS `{container_name}` has been unsuspended by an admin.\nYou can now manage it again.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
...
async def suspension_logs(ctx, container_name: str = None):
    if container_name:
        ...
        if not history:
            await ctx.send(embed=create_info_embed("No Suspensions", f"No GloryVPS suspension history for `{container_name}`."))
            return
        embed = create_embed("GloryVPS Suspension History", f"For `{container_name}`")
        text = []
        ...
        if not all_logs:
            await ctx.send(embed=create_info_embed("No Suspensions", "No GloryVPS suspension events recorded."))
            return
        logs_text = "\n".join(all_logs)
        chunks = [logs_text[i:i+1024] for i in range(0, len(logs_text), 1024)]
        for idx, chunk in enumerate(chunks, 1):
            embed = create_embed(f"GloryVPS Suspension Logs (Part {idx})", f"Global suspension events (newest first)")
...
async def apply_permissions(ctx, container_name: str):
    ...
    try:
        ...
        await ctx.send(embed=create_success_embed("Permissions Applied", f"Advanced permissions applied to GloryVPS `{container_name}`. Docker-ready!"))
    except Exception as e:
...
async def snapshot_vps(ctx, container_name: str, snap_name: str = "snap0"):
    ...
    try:
        ...
        await ctx.send(embed=create_success_embed("Snapshot Created", f"Snapshot '{snap_name}' created for GloryVPS `{container_name}`."))
    except Exception as e:
                ...
                for uid, lst in vps_data.items():
                    for vps in lst:
                        if vps['container_name'] == container_name:
                            ...
                            save_vps_data()
                            break
                await inter.followup.send(embed=create_success_embed("Snapshot Restored", f"Restored '{snap_name}' for GloryVPS `{container_name}`."))
            except Exception as e:
...
async def show_help(ctx):
    ...
    embed = create_embed("📚 GloryVPS Command Help - User Commands", "GloryVPS Manager Commands:", 0x1a1a1a)
    user_commands = [
        ("!ping", "Check GloryVPS bot latency"),
        ("!uptime", "Show host uptime"),
        ("!myvps", "List your GloryVPS"),
        ("!manage [@user]", "Manage your VPS or another user's VPS (Admin only)"),
        ("!share-user @user <vps_number>", "Share GloryVPS access"),
        ("!share-ruser @user <vps_number>", "Revoke GloryVPS access"),
        ("!manage-shared @owner <vps_number>", "Manage shared GloryVPS")
    ]
    user_commands_text = "\n".join([f"**{cmd}** - {desc}" for cmd, desc in user_commands])
    add_field(embed, "👤 User Commands", user_commands_text, False)
    await ctx.send(embed=embed)
    if is_user_admin:
        embed = create_embed("📚 GloryVPS Command Help - Admin Commands", "GloryVPS Manager Commands:", 0x1a1a1a)
        admin_commands = [
            ("!lxc-list", "List all LXC containers"),
            ("!create <ram_gb> <cpu_cores> <disk_gb> @user", "Create VPS with OS selection"),
            ("!delete-vps @user <vps_number> [reason]", "Delete user's GloryVPS"),
            ("!add-resources <container> [ram] [cpu] [disk]", "Add resources to GloryVPS"),
            ("!resize-vps <container> [ram] [cpu] [disk]", "Resize GloryVPS resources"),
            ("!suspend-vps <container> [reason]", "Suspend GloryVPS"),
            ("!unsuspend-vps <container>", "Unsuspend GloryVPS"),
    ...
    if is_user_main_admin:
        embed = create_embed("📚 GloryVPS Command Help - Main Admin Commands", "GloryVPS Manager Commands:", 0x1a1a1a)
        main_admin_commands = [
        ...
        embed.set_footer(text="GloryVPS Manager • Auto-suspend high-usage only • Whitelist support • Multi-OS • Enhanced monitoring • Docker-ready VPS • Snapshots")
        await ctx.send(embed=embed)

# Command aliases for typos
@bot.command(name='mangage')
async def manage_typo(ctx):
    await ctx.send(embed=create_info_embed("Command Correction", "Did you mean `!manage`? Use the correct GloryVPS command."))
...
async def stats_alias(ctx):
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        ...
        await ctx.send(embed=create_error_embed("Access Denied", "This GloryVPS command requires admin privileges."))
